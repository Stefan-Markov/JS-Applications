...
let hostPort = 'http://localhost:3000';
const host = hostPort; // Application host (NOT service host - that can be anything)
const DEBUG = false;
...
describe('E2E tests', function () {
    ...
    describe('User Profile Page [ 10 Points ]', async () => {
        ...
        // Login user
        beforeEach(async () => {
            page.route(loginEndpoint, route => route.fulfill(json({
                _id: '0002',
                gender: 'female',
                username,
                email,
        ...
        it('check profile page information - with 2 memes [ 2.5 Points ]', async () => {
            ...
            expect(memes[1]).to.contains('my description es');
